...
import { ChevronDown, ChevronUp, Plus, Trash2, Check, Edit2, Grip, TrendingUp, Info } from 'lucide-react';
...
interface ExerciseCardEnhancedProps {
  ...
  onUpdateIntensity: (exerciseId: string, setId: string, intensity: 'easy' | 'medium' | 'hard') => void;
...
export function ExerciseCardEnhanced({
  ...
  const getIntensityColor = (intensity?: 'easy' | 'medium' | 'hard') => {
    switch (intensity) {
      case 'easy':
        return 'bg-[#22C55E]';
      case 'medium':
  ...
  const getIntensityLabel = (intensity?: 'easy' | 'medium' | 'hard') => {
    switch (intensity) {
      case 'easy':
        return 'Easy';
      case 'medium':
  ...
  const getProgressionSuggestion = (intensity?: 'easy' | 'medium' | 'hard', weight?: string) => {
    if (!intensity || !weight) return null;
    const currentWeight = parseFloat(weight);
    if (isNaN(currentWeight)) return null;

    switch (intensity) {
      case 'easy':
        return `+${2.5}kg next time`;
      case 'medium':
    ...
    <div className="rounded-xl bg-[#151923] border border-[#151923] overflow-hidden">
      ...
      {isExpanded && (
        <div className="p-4 space-y-3">
          ...
          <div className="bg-gradient-to-r from-[#22C55E]/10 to-[#00D1FF]/10 border border-[#22C55E]/20 rounded-lg p-3 mb-2">
            <div className="flex items-start gap-2">
              <TrendingUp className="w-4 h-4 text-[#22C55E] mt-0.5 flex-shrink-0" />
              <div className="flex-1">
                <p className="text-xs text-gray-300 leading-relaxed">
                  <span className="text-[#22C55E] font-medium">Auto-Progression:</span>{' '}
                  Mark sets as <span className="text-[#22C55E]">Easy</span> (+2.5kg next),{' '}
                  <span className="text-yellow-500">Medium</span> (maintain), or{' '}
                  <span className="text-red-500">Hard</span> (-2.5kg next)
                </p>
              </div>
            </div>
          </div>
          ...
          {exercise.sets.map((set, index) => (
            <div key={set.id} className="space-y-2">
                ...
                <div className="col-span-3">
                  <div className="flex gap-1">
                    {(['easy', 'medium', 'hard'] as const).map((intensity) => (
                      <button
                        key={intensity}
                        onClick={() => onUpdateIntensity(exercise.id, set.id, intensity)}
                        disabled={!set.completed}
              ...
              {set.completed && set.intensity && (
                <div className="ml-14 text-xs text-gray-500 italic flex items-center gap-1">
                  <TrendingUp className="w-3 h-3" />
                  <span>{getProgressionSuggestion(set.intensity, set.weight)}</span>
                </div>
              )}
            </div>
          ))}
